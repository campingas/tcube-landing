...
Sanzo Wada (1883–1967) was a Japanese painter, Oscar-winning costume designer, and the founder of the Japan Standard Colour Association. His *A Dictionary of Color Combinations* (originally published 1933, reissued Seigensha 2010) catalogs 348 colour combinations across 159 named colours. <br>
Reference: [wscolors.com](https://wscolors.com) · [sanzo-wada.dmbk.io](https://sanzo-wada.dmbk.io)

His method is not a formula — it's an eye. Every combination in his dictionary was chosen because it simply *works*. His reds are terracotta, not fire. His blues lean slate, not sky. His yellows are ochre-gold, not lemon. The palette never ages because it was never trend-dependent.
...
Derived from Wada's catalogue. Combination reference: adapted from Combination 214-variant (warm ink + organic accent family).

| Token | Wada # | Wada Name | HEX | Role |
|---|---|---|---|---|
| `wada-ink` | #88 (deepened) | Turquoise Green | `#1a2e2a` | Default background — 60% of all surfaces |
| `wada-teal` | #101 | Benzol Green | `#00978d` | Brand primary, headings, logo mark |
| `wada-salmon` | #50 | Ochraceous Salmon | `#d8a37b` | Warm support, cards, hardware illustration |
| `wada-rufous` | #68 | Orange Rufous | `#c16b27` | CTA buttons, accent, hover, hardware LED |
| `wada-paper` | #39 | Sulpher Yellow | `#f5ecc2` | Primary text on dark, near-white ground |
| `wada-moss` | #96 | Andover Green | `#6d7e77` | Secondary text, dividers, muted labels |
| `wada-slate` | #110 | Deep Slate Olive | `#253122` | Darkest surface, footer, deep card background |
...
| Weight | Role | Tokens |
|---|---|---|
| 60–70% | Dominant — backgrounds, large surfaces | `wada-ink`, `wada-slate` |
| 20–30% | Supporting — mid-weight elements | `wada-teal`, `wada-salmon` |
| 5–10% | Accent — interactions, focus, highlights | `wada-rufous` |

`wada-paper` and `wada-moss` sit outside this rule — they are text colours, not surface colours.
...
- **Background**: `wada-ink` always. Never `#000000` (too harsh), never `#0d1117` (cold/GitHub-blue).
- **Primary text**: `wada-paper`. Never `#ffffff` — too clinical for a child-facing product.
- **Secondary text / labels / captions**: `wada-moss`. Only at ≥18px bold or ≥24px regular — contrast is borderline at small sizes.
- **CTA buttons**: `wada-rufous` background, `wada-paper` text. On hover: invert to `wada-teal` background.
- **Cards and raised surfaces**: `wada-slate` — one step deeper than `wada-ink`, creates depth without drama.
- **Hardware illustrations**: `wada-salmon` dominant fill, `wada-rufous` accent detail. The Raspberry Pi and T-Cube enclosure should feel warm and tactile — not a glossy product render.
...
|---|---|---|---|
| `wada-paper` on `wada-ink` | ~10.2:1 | AAA | Body text — safe at all sizes |
| `wada-paper` on `wada-slate` | ~11.4:1 | AAA | Cards — fully safe |
| `wada-teal` on `wada-ink` | ~4.7:1 | AA | Headings, large text |
| `wada-rufous` on `wada-ink` | ~3.8:1 | AA large | Buttons, badges — ≥18px only |
| `wada-moss` on `wada-ink` | ~3.2:1 | AA large | Labels only — ≥18px bold or ≥24px regular |
| `wada-paper` on `wada-rufous` | ~3.9:1 | AA large | CTA button text — acceptable |

The contrast ratios for `wada-rufous` and `wada-moss` on dark surfaces are borderline. Do not use either as body copy. If you need a contrast-safe accent at small sizes, use `wada-teal`.
...
**The one-word rule**: in code review and in conversation, use the scale name — not the pixel value. *"The language flag grid tiles are `tile`, not `chip`"*. The name carries intent; the pixel value is just today's output.
...
```css
h1 { font-size: clamp(2.5rem, 8vw, 6.875rem); }
h2 { font-size: clamp(1.75rem, 5vw, 4.25rem); }
```
...
> ✓ "T-Cube reads aloud in Vietnamese, English, and more."
...
|---|---|---|
| "Revolutionary learning experience" | "A story for every lesson." | Specificity beats superlative |
| "Our platform leverages AI-powered TTS" | "T-Cube reads aloud in Vietnamese, English, and more." | Name the action, name the language |
...
| "Seamless multilingual support" | "Vietnamese first. Then English, French, Chinese." | Sequence and specificity |
...
English is the primary language. Vietnamese is a secondary layer on the same page. Rules:

- Bilingual labels are acceptable (`Get started / Bắt đầu`).
- Bilingual sentences are not (`"T-Cube giúp children học ngôn ngữ mới"`).
- When writing English for this codebase, write as if it will be translated into Vietnamese immediately — no English idioms, no culture-specific wordplay, no puns that don't survive translation.
- Tone in Vietnamese should be warm and direct (`bạn`, not formal), matching the four-word model above.
...
**Canonical answer:**
> T-Cube is a small device that reads stories and lessons aloud to children — in English first, then in other languages — so they learn by listening.

---

## 4. The Jobs Checklist

Steve Jobs' design review was a single repeated question: *"Is this the best we can do?"* — followed by silence until someone answered honestly.
...
This is Coco Chanel's mirror rule applied to UI: *"Before leaving the house, look in the mirror and remove one accessory."* Run it for every section, every release.

---

### Gate 3 — The stranger test

Show the section to someone who has never heard of T-Cube. Give them exactly 5 seconds. Then cover the screen and ask: *"What does this company do?"*
...
> T-Cube is a small device that reads stories and lessons aloud to children — in Vietnamese first, then in other languages — so they learn by listening.
...
Read every text string visible on the page. For each one, ask: *"If I removed this, would anyone notice something missing?"*
...
COLOURS  (all wada-* in Tailwind)
  wada-ink     #1a2e2a   background, 60% of surfaces
  wada-teal    #00978d   brand primary, headings, logo
  wada-salmon  #d8a37b   warm support, cards, hardware
  wada-rufous  #c16b27   CTA buttons, accent, LED
  wada-paper   #f5ecc2   primary text on dark
  wada-moss    #6d7e77   secondary text, dividers (≥18px only)
  wada-slate   #253122   darkest surface, footer
...
VOICE
  ...
  Active voice. Short sentences. Vietnamese-first.
...
*Colour references verifiable at [wscolors.com](https://wscolors.com) (Wada Sanzo digital catalogue). Scale formula: φ = 1.618, object seed = 24px, type seed = 16px, spacing seed = 8px, divine leap god = hero × 2.*
...
```typescript
wada: {
  ink:    '#1a2e2a',  // dominant — 60% of visual surface
  teal:   '#00978d',  // brand primary — Wada #101 Benzol Green
  salmon: '#d8a37b',  // supporting warm — Wada #50 Ochraceous Salmon
  rufous: '#c16b27',  // accent / CTA — Wada #68 Orange Rufous
  paper:  '#f5ecc2',  // near-white ground — Wada #39 Sulpher Yellow
  moss:   '#6d7e77',  // muted mid — Wada #96 Andover Green
  slate:  '#253122',  // deepest anchor — Wada #110 Deep Slate Olive
},
```
...
- **Background default**: `wada-ink`. Dark-first design; no light mode toggle in v1.
- **Primary text**: `wada-paper`. Never pure white (`#ffffff`) — too clinical.
- **Secondary / muted text**: `wada-moss`. Used for metadata, labels, captions.
- **Interactive elements (links, buttons, highlights)**: `wada-rufous` with `wada-teal` on hover.
- **Cards and raised surfaces**: `wada-slate` — one step darker than `wada-ink` to create depth without contrast drama.
- **Hardware illustration fills**: `wada-salmon` dominant, `wada-rufous` accent. The Raspberry Pi and T-Cube enclosure should feel warm, tactile, handmade — not glossy render.
- Never use `wada-teal` and `wada-rufous` as neighbours at equal weight — they compete. One dominates, one accents.
- Sanzo proportions enforced: in any section, count the visual weight. Dominant (ink/paper) > supporting (teal/salmon) > accent (rufous). Flip at your own risk.
...
Reference as `size-stamp`, `size-chip`, etc. via a plugin or as CSS custom properties:
...
#### Typography scale (same φ root)

Body base: **16px** (not in the square element scale — type lives on its own φ ladder seeded from 16).

| Token | px | rem | Role |
|---|---|---|---|
| `text-xs` | 10 | 0.625 | Legal, footnote |
| `text-sm` | 16 | 1.0 | Body, paragraph |
| `text-md` | 26 | 1.625 | Lead paragraph, card body |
| `text-lg` | 42 | 2.625 | Section subheading (h3) |
| `text-xl` | 68 | 4.25 | Section heading (h2) |
| `text-2xl` | 110 | 6.875 | Hero headline (h1, clamp-limited on mobile) |

Apply `clamp()` to the two largest to prevent overflow at narrow viewports:
...
#### Spacing scale (φ-derived, rounded)

Seed: **8px**. Gaps, padding, and section rhythm all live on this ladder.

| Token | px | Use |
|---|---|---|
| `space-1` | 8 | Tight inline gap |
| `space-2` | 13 | Component inner padding |
| `space-3` | 21 | Card padding |
| `space-5` | 34 | Between related elements |
| `space-8` | 55 | Section inner margin |
| `space-13` | 89 | Between sections |
| `space-21` | 144 | Hero vertical breathing room |

These map exactly to Fibonacci-adjacent values because φ and Fibonacci converge. The numbers feel inevitable, not arbitrary.
